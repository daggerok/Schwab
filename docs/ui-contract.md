...
- **All ETFs** (`All`) plus one tab per Schwab asset class (category: U.S. Equities, International Equities, Global Equities, Fixed Income, Real Estate, Money Market).
...
| Key | Content |
| --- | --- |
| `schwab-selected-etfs` | JSON array of selected ETF tickers |
| `schwab-active-fund` | active fund ticker (removed when none) |
| `schwab-blacklisted-etfs` | JSON array of blacklisted tickers |
| `schwab-tab-sorts` | JSON map `tab -> { key, dir }` of **explicitly chosen** sorts |
| `schwab-tab-filters` | JSON map `tab -> query` of **explicitly entered** filters |
| `schwab-site-state` | auxiliary state: `sheetFilter` mirror of the per-tab filter map plus the last `activeTab` |
| `schwab-theme` | `dark` / `light` |
| `schwab-searches` | **legacy** pre-rename filter key; migrated at boot and removed on first write |
...
- Every tab remembers its own search query in `schwab-tab-filters`.
...
- `schwab-site-state.sheetFilter` mirrors the dedicated filter map so
...
- `schwab-selected-etfs` / `schwab-active-fund` in localStorage.
...
- The Watchlist table mirrors this: `Loading holdings of N selected ETFs…`
  ...
  not available yet. Run bun ./scripts/update-data.ts to refresh the Schwab feed.`
...
The Schwab feed publishes one `Identifier` column per holding row (the BBG
FIGI from the issuer holdings CSV, or the CUSIP/ISIN recorded by the SEC
N-PORT-P fallback) plus `Ticker` and `Name`; there are no separate
CUSIP/ISIN/SEDOL columns.
...
**Schwab extension:** the issuer holdings CSV labels bond positions with
generic instrument symbols (`TNOTE`, `T`, `WIT`, …) that are not exchange
tickers. The data build blanks the `Ticker` of every row that carries a
maturity date, so Treasury notes are keyed by their FIGI (`D:BBG…`) and never
collide with a real equity ticker such as `T` (AT&T). All-zero identifiers
(`000000000`, published by some EDGAR N-PORT filings) are treated as missing
as well, so those rows fall through to the published name instead of
collapsing into a single garbage key (`D:000000000` must never appear in the
aggregation).
...
cash positions (e.g. the `USD` / `US DOLLAR` rows published in the feed), futures/swaps/
...
- `api/schwab/index.json` — catalog; `funds[].holdings`/`funds[].history`
  agree with the per-fund manifests (validated by the acceptance tests' feed
  oracle, test 15).
- `api/schwab/funds/<TICKER>/meta.json` — `holdings.pages[]` and
...
file-backed `fetch` over `api/schwab/`) and covers:
...
12. identifier fallbacks for bonds (FIGI-keyed, generic Treasury symbols
    never leak into the ticker namespace), cash (`USD`), placeholder
    identifiers and numeric local tickers are kept;
