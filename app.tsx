/**
 * @file Schwab Watchlist Application
 * Client-side static feed viewer for api/schwab/** with multi-ETF Watchlist
...
const INDEX_URL = './api/schwab/index.json';
const THEME_KEY = 'schwab-theme';
const SELECTED_KEY = 'schwab-selected-etfs';
const BLACKLIST_KEY = 'schwab-blacklisted-etfs';
const ACTIVE_FUND_KEY = 'schwab-active-fund';
const FILTERS_KEY = 'schwab-tab-filters';
const LEGACY_FILTERS_KEY = 'schwab-searches'; // pre-rename key, migrated at boot
const SORTS_KEY = 'schwab-tab-sorts';
const SITE_STATE_KEY = 'schwab-site-state';
...
const COLUMN_TOOLTIPS: Record<string, string> = {
  ...
  'Fund Name': 'Fund Name — Official Schwab Asset Management product-page name of the exchange-traded fund (ETF).',
  Category: 'Category — the Schwab Asset Management product finder groups its ETFs by asset class (U.S. Equities, International Equities, Global Equities, Fixed Income, Real Estate, Money Market); the tab shows that asset class, the product-finder grouping is kept in meta.json.',
  Name: 'Security Name — Full registered legal name of the company or underlying financial asset.',
  Identifier: 'Identifier — BBG FIGI as published in the schwabassetmanagement.com holdings CSV (CUSIP / ISIN when the SEC N-PORT-P fallback supplied the sheet). Positions without an exchange ticker (bonds, cash, futures) are identified in the Watchlist by this.',
  ...
  Type: 'Category — the asset class from the Schwab Asset Management product finder (see the Category column). Same source as the category tabs.',
  Expense: 'Gross Expense Ratio — Total annual fund operating expenses as a % of assets.',
  'Dividend Yield': 'Dividend Yield — the Distribution Yield (TTM) published on the Schwab Asset Management product page when present; otherwise indicated (latest distribution per share x payments per year / NAV) from the official distribution history.',
  'SEC Yield': 'SEC Yield (30-Day) — The 30-day SEC yield as published on the official Schwab Asset Management product page; "—" when that page does not publish one.',
  'YTD Return': 'YTD Return — Official Schwab NAV total return year-to-date (product page, month-end series) where published; otherwise market-price return since the start of the year computed from adjusted closes (Yahoo).',
  'TR 1Y': 'TR 1Y (1-Year Total Return) — Official Schwab NAV total return over the past year (product page, month-end series) where published, otherwise adjusted market-price return from Yahoo.',
  ...
  'CAGR 3Y': 'CAGR 3Y (3-Year Compound Annual Growth Rate) — Official Schwab annualized NAV total return over 3 years (product page) where published, otherwise computed from adjusted closes.',
  'CAGR 5Y': 'CAGR 5Y (5-Year Compound Annual Growth Rate) — Official Schwab annualized NAV total return over 5 years (product page) where published, otherwise computed from adjusted closes.',
  'CAGR 10Y': 'CAGR 10Y (10-Year Compound Annual Growth Rate) — Official Schwab annualized NAV total return over 10 years (product page) where published, otherwise computed from adjusted closes.',
  YTD: 'YTD total return — official Schwab NAV return (month-end) where published, otherwise market-price return from adjusted closes.',
  '1Y': '1-year official Schwab NAV total return where published, otherwise adjusted market-price return, month-end series.',
  '3Y': '3-year average annual return (CAGR), official Schwab NAV total return where published, otherwise Yahoo adjusted close.',
  '5Y': '5-year average annual return (CAGR), official Schwab NAV total return where published, otherwise Yahoo adjusted close.',
  '10Y': '10-year average annual return (CAGR), official Schwab NAV total return where published, otherwise Yahoo adjusted close.',
  'SI Ann.': 'Since-inception annualized return, official Schwab NAV total return where published, otherwise Yahoo adjusted close.',
  ...
  Frequency: 'Frequency — sortable payment cadence coded by scripts/update-data.ts (frequencyCode in index.json) from the official Schwab distribution history, Yahoo dividends as fallback: 01 - Monthly, 04 - Quarterly, 06 - Semi-annually, 12 - Annually; 00 denotes unavailable/unknown and 99 denotes irregular.',
  'Ex-Date': 'Ex-dividend date of the latest distribution.',
  Dividend: 'Latest dividend per share.',
  Coupon: 'Bond annual coupon rate (%).',
  Maturity: 'Bond maturity date.',
  'Market Value': 'Position market value in USD — derived by the feed as Percent of Assets x Total Net Assets because the Schwab holdings CSV publishes weights, not values (N-PORT-P value when that fallback supplied the sheet).',
...
function exportFileName(scope: string, extension: string): string {
  ...
  return `schwab-${scope.toLowerCase().replace(/\s+/g, '-')}-${stamp}.${extension}`;
}
...
// =========================================================================
// 4. Static API loading & paginated sheets (api/schwab/**, iShares-style)
// =========================================================================
...
function normalizeFundRow(fund: IndexFund): FundRow {
  ...
  const row: any = {
    ...
    secYield: metrics.secYield ?? null, // official Schwab product pages publish the 30-day SEC yield for most funds.
...
async function loadCatalog(): Promise<void> {
  setStatus('Loading Schwab ETF data from api/schwab/index.json…', 'info');
...
async function loadFundMeta(ticker: string): Promise<any> {
  ...
  const request = (async () => {
    try {
      const meta = await fetchJson(`./api/schwab/funds/${encodeURIComponent(ticker)}/meta.json`);
...
async function fetchPage(ticker: string, pagePath: string): Promise<{ headers: string[]; rows: string[][] }> {
  ...
  const page = await fetchJson(`./api/schwab/funds/${encodeURIComponent(ticker)}/${path}`);
  ...
  if (!state.selected.size) {
    ...
    el.tableBody.innerHTML = `<tr><td colspan="8" class="py-12 text-center text-slate-400 dark:text-slate-500">${
      loading
        ...
        : 'Holdings data is not available yet. Run bun ./scripts/update-data.ts to refresh the Schwab feed.'
...
function renderOverviewTable(fund: FundRow): void {
  ...
  const overview: Array<{ section: string; metric: string; value: unknown }> = [
    ...
    { section: 'Distributions', metric: 'SEC Yield (30-day)', value: meta && meta.yields ? (meta.yields.secYieldText || '—') : (fund.secYield === null || fund.secYield === undefined ? 'not published on the official Schwab Asset Management product page for this fund' : `${fund.secYield.toFixed(2)}%`) },
  ...
  el.tickerCount.textContent = fund.ticker;
  renderSubtitle(`${fund.ticker} overview · ${rows.length} metrics. Returns are the official Schwab NAV total returns where published (Yahoo adjusted closes otherwise).`);
}
...
function renderDistributionsTable(fund: FundRow): void {
  ...
  el.tickerCount.textContent = fund.ticker;
  renderSubtitle(`${fund.ticker} distributions · official Schwab distribution history (ex-date, total distribution per share; Yahoo dividends when the CSV is unavailable); frequency is inferred from the cadence.`);
}
...
function renderSubtitle(text?: string): void {
  ...
  const countsText = state.counts
    ...
    : '';
  const base = text ? String(text) : 'Search Schwab ETFs, select ETFs via the “Use” checkbox, then use the Watchlist tab.';
  ...
  el.subtitle.innerHTML = `
    ...
    <span class="block sm:inline">·${generated ? ` updated ${escapeHtml(generated)}` : ''}${countsText ? ` · ${escapeHtml(countsText)}.` : '.'} Data: <a href="./api/schwab/index.json" target="_blank" rel="noopener noreferrer" class="font-semibold text-blue-600 dark:text-blue-400 hover:underline">api/schwab/index.json</a> generated from <a href="https://www.schwabassetmanagement.com/products/etfs" target="_blank" rel="noopener noreferrer" class="font-semibold text-blue-600 dark:text-blue-400 hover:underline">schwabassetmanagement.com product pages + CSV downloads</a> + Yahoo Finance</span>
  `;
...
function init(): void {
  ...
  void loadCatalog().catch(error => {
    ...
    setStatusRow(`Unable to load api/schwab/index.json: ${message}. Run bun ./scripts/update-data.ts and serve the folder (for example bunx serve . -p 1234).`, 'error');
  });
}
