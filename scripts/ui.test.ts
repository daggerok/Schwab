/**
 * Acceptance tests for the Schwab ETF UI contract (index.html + app.tsx).
 *
 ...
 * that serves the actual generated feed from api/schwab/. Expected
...
test("2. header Use check selects exactly the filtered ETFs", async () => {
  const app = await bootFresh();
  setSearch(app, "treasury"); // matches SCHO, SCHQ, SCHR only
  const visible = app.run<string[]>("visibleCatalogRows().map((f) => f.ticker)").sort();
  expect(visible).toEqual(["SCHO", "SCHQ", "SCHR"]);
...
test("2b. filtered bulk selection immediately opens the selected-fund panel", async () => {
  const app = await bootFresh();
  setSearch(app, "large"); // the user-facing regression: select all visible matches
  const visible = app.run<string[]>("visibleCatalogRows().map((f) => f.ticker)").sort();
...
test("3. header Use uncheck removes only visible tickers; hidden selections survive", async () => {
  ...
  const visible = app.run<string[]>("visibleCatalogRows().map((f) => f.ticker)").sort();
  expect(visible).not.toContain("SCHD");

  setChecked(headerCheckbox(app), true);
  expect(selectedTickers(app)).toEqual([...visible, "SCHD"].sort());
  // checked state: every visible row selected -> header box is checked
  expect(headerCheckbox(app).checked).toBe(true);

  setChecked(headerCheckbox(app), false);
  expect(selectedTickers(app)).toEqual(["SCHD"]); // hidden selection survives
...
test("4. All ETFs pill selects the whole non-blacklisted catalog without navigating", async () => {
  ...
  expect(selected).toEqual(all.filter((t) => t !== "SCHD").sort());
...
test("5. selecting one ETF shows Watchlist Loading then the exact count", async () => {
  ...
  app.stats.latency = (url) => (url.includes("/SCHX/") ? 60 : 0);
  ...
  const expected = expectedWatchlist(["SCHX"]).size;
...
test("6. rapid overlapping selections load without duplicate or skipped pages", async () => {
  const app = await bootFresh();
  app.stats.latency = (url) => (url.includes("/SCHX/") || url.includes("/SCHG/") ? 40 : 0);

  toggleRow(app, "SCHX");
  toggleRow(app, "SCHG"); // second selection while the first is still in flight

  const oracle = expectedWatchlist(["SCHX", "SCHG"]);
  await waitWatchlistCount(app, oracle.size);

  // every holdings page fetched exactly once — no duplicates, no gaps
  for (const ticker of ["SCHX", "SCHG"]) {
    const pages: string[] = feedJson(`funds/${ticker}/meta.json`).holdings.pages;
    for (const page of pages) {
      expect(app.stats.counts.get(`./api/schwab/funds/${ticker}/${page}`) ?? 0).toBe(1);
    }
    // meta.json requests are deduplicated per ticker while in flight
    expect(app.stats.counts.get(`./api/schwab/funds/${ticker}/meta.json`) ?? 0).toBe(1);
  }
...
test("7. deselecting an ETF updates subtitle, tabs and Watchlist immediately", async () => {
  ...
  toggleRow(app, "SCHX");
  toggleRow(app, "SCHG");
  const both = expectedWatchlist(["SCHX", "SCHG"]).size;
  await waitWatchlistCount(app, both);
  expect(watchlistTabLabel(app)).toBe(String(both));

  toggleRow(app, "SCHG"); // deselect — everything below must be synchronous

  const onlySchx = expectedWatchlist(["SCHX"]).size;
  expect(app.run<number>("getDedupedWatchlistRows().length")).toBe(onlySchx);
  expect(watchlistTabLabel(app)).toBe(String(onlySchx));
  expect(app.el("app-subtitle").innerHTML).toContain("1 selected");
  expect(app.el("app-subtitle").innerHTML).not.toContain('data-activate-fund="SCHG"');
  expect(app.el("selected-tabs-bar").innerHTML).not.toContain("SCHG Overview");
  expect(app.el("selected-tabs-bar").innerHTML).toContain("SCHX Overview");
  expect(app.run("state.activeFundTicker")).toBe("SCHX");
}, 240000);
...
test("9. reload restores selection, active fund and rebuilds the Watchlist", async () => {
  ...
  expect(reloaded.el("selected-tabs-bar").innerHTML).toContain("SCHG Overview");
}, 300000);
...
test("10. Holdings, History, Overview and Distributions render real rows", async () => {
  ...
  await clickTab(app, "detail:holdings");
  const firstPageRow = feedJson("funds/SCHX/holdings/001.json").rows[0];
  ...
  for (const page of feedJson("funds/SCHX/meta.json").holdings.pages) {
    expect(app.stats.counts.get(`./api/schwab/funds/SCHX/${page}`) ?? 0).toBe(1);
  }
  ...
  await until(() => app.run<number>("sheetState.get('SCHX:history').nextPage") >= 1, 30000);
  const historyRow = feedJson("funds/SCHX/history/001.json").rows[0];
  await until(() => bodyRowHtml(app).includes(String(historyRow.Date)));
  expect(app.stats.counts.get("./api/schwab/funds/SCHX/history/001.json") ?? 0).toBe(1);

  const scroll = app.el("table-scroll");
  scroll.scrollHeight = 20000;
  scroll.scrollTop = 20000 - scroll.clientHeight - 100;
  scroll.dispatch("scroll", { target: scroll });
  await until(() => app.run<number>("sheetState.get('SCHX:history').nextPage") === 2, 30000);
  expect(app.stats.counts.get("./api/schwab/funds/SCHX/history/002.json") ?? 0).toBe(1);
  const secondHistoryRow = feedJson("funds/SCHX/history/002.json").rows[0];
  ...
  const distributions = feedJson("funds/SCHX/meta.json").distributions;
...
test("11. a fund whose files fail to load shows an explanatory state", async () => {
  ...
  const priorName = String(feedJson("funds/SCHX/holdings/001.json").rows[0].Name);
  app.stats.blocked.add("funds/SCHG/");
  toggleRow(app, "SCHG"); // becomes the active fund; its files fail to load
  toggleRow(app, "SCHX"); // deselect SCHX so SCHG is the only remaining fund
  await until(() => app.run("state.activeFundTicker") === "SCHG");
  await clickTab(app, "detail:holdings"); // must show the failure state, not SCHX's table
...
test("12. bond rows fall back to identifiers; cash and zero-weight rows are kept", async () => {
  const app = await bootFresh();
  // SCHZ: bond rows without exchange tickers (the Schwab CSV publishes generic
  //       instrument labels such as TNOTE for them, so the feed blanks the
  //       Ticker and the BBG FIGI Identifier becomes the key);
  // SCHD: the cash position row (Symbol USD / US DOLLAR — kept, not dropped);
  // SCHE: foreign listings, including numeric local tickers.
  toggleRow(app, "SCHZ");
  toggleRow(app, "SCHD");
  toggleRow(app, "SCHE");
  const oracle = expectedWatchlist(["SCHZ", "SCHD", "SCHE"]);
  ...
  // generic bond labels never leak into the ticker namespace: no Treasury
  // position may collide with a real exchange ticker such as T (AT&T)
  const bondRows = fundHoldingRows("SCHZ");
  expect(bondRows.length).toBeGreaterThan(500);
  for (const row of bondRows) {
    if (String(row.Maturity ?? "").trim() && String(row.Maturity) !== "-") {
      expect(String(row.Ticker ?? "-")).toBe("-");
    }
  }
  expect([...oracle.values()].some((r) => r.key === "T:TNOTE")).toBe(false);

  // placeholder identifiers never become keys
  expect([...oracle.values()].some((r) => r.key === "D:000000000")).toBe(false);
  expect([...oracle.values()].some((r) => /^[A-Z]:(-|--|—|N\/A)$/.test(r.key))).toBe(false);

  // cash positions are kept, not aggressively dropped
  expect(oracle.has("T:USD")).toBe(true);
  expect(shown.has("USD")).toBe(true);

  // zero-weight rows with otherwise valid data are kept (when the feed has any)
  const zeroWeight = [...oracle.values()].find((r) => r.hasWeight && Math.abs(r.weightSum) < 1e-12);
  if (zeroWeight) expect(shown.has(zeroWeight.shown)).toBe(true);
  ...
  expect(app.run("JSON.stringify(positionDedupeKey({ Ticker: '-', Identifier: 'BBG000BPD257', Name: 'MERCK & CO INC' }))")).toBe(
    JSON.stringify({ key: "D:BBG000BPD257", shown: "BBG000BPD257" }),
  );
  expect(app.run("JSON.stringify(positionDedupeKey({ Ticker: 'USD', Name: 'US DOLLAR', Weight: '0.012' }))")).toBe(
    JSON.stringify({ key: "T:USD", shown: "USD" }),
  );
}, 300000);
...
test("14. malformed localStorage is sanitized and boot still succeeds", async () => {
  ...
  expect(app.run<number>("state.funds.length")).toBe(catalogTickers().length);
  expect(app.run<string[]>("[...state.selected]")).toEqual([]);
...
test("16. per-tab filter persistence: each tab keeps its own search query independently", async () => {
  ...
  // type a filter on Holdings tab (SCHX holds the Alphabet share classes;
  // the expected count is computed from the feed, not hardcoded)
  const alphabetRows = fundHoldingRows("SCHX").filter((row) => Object.values(row).join(" ").toLowerCase().includes("alphabet")).length;
  expect(alphabetRows).toBeGreaterThan(0);
  setSearch(app, "alphabet");
  ...
  expect(app.el("search-input").value).toBe("");
  setSearch(app, "SCHX");
  expect(app.el("search-input").value).toBe("SCHX");
  ...
  expect(reloaded.el("ticker-count").textContent).toBe(`${catalogTickers().length} ETFs`);
}, 300000);
...
test("17. #search-clear-btn visibility, active-tab clearing, and immediate re-render", async () => {
  ...
  setSearch(app, "fundamental"); // matches the 6 Fundamental Index ETFs
  expect(clearBtn.classList.contains("hidden")).toBe(false);
  expect(app.run<number>("visibleCatalogRows().length")).toBe(6);
  expect(app.el("ticker-count").textContent).toBe("6 ETFs");

  clearBtn.click();
  expect(input.value).toBe("");
  expect(clearBtn.classList.contains("hidden")).toBe(true);
  expect(app.el("ticker-count").textContent).toBe(`${catalogTickers().length} ETFs`);
