/**
 * Headless harness for the Schwab UI (index.html + app.tsx).
 *
 ...
 * generated feed from api/schwab/. That lets bun test exercise the exact
...
const API_ROOT = path.join(REPO_ROOT, "api", "schwab");
...
function createFeedFetch(stats: FetchStats) {
  return async function fetchStub(input: string): Promise<any> {
    ...
    if (!relative.startsWith("api/schwab/")) return notFound();
 ...
 * feed) also count as missing (contract §5, Schwab extension).
 */
